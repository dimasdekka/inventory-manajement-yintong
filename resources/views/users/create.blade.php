    ...
    <form action="{{ route('users.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row g-3 mb-4">
            <div class="col-12 col-md-6">
                ...
                <input type="text" class="form-control form-control-custom w-100 @error('nama') is-invalid @enderror" id="nama" name="nama" value="{{ old('nama') }}" placeholder="Contoh: Budi Santoso" required>
                @error('nama')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="col-12 col-md-6">
                <label for="email" class="form-label-custom">Alamat Email <span class="text-danger">*</span></label>
                <input type="email" class="form-control form-control-custom w-100 @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email') }}" placeholder="Contoh: budi@instansi.com" required>
                @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                <div class="form-text text-muted">Digunakan sebagai email login ke sistem.</div>
            </div>
        </div>

        <div class="row g-3 mb-4">
            <div class="col-12 col-md-4">
                <label for="password" class="form-label-custom">Kata Sandi Login <span class="text-danger">*</span></label>
                <input type="password" class="form-control form-control-custom w-100 @error('password') is-invalid @enderror" id="password" name="password" placeholder="Minimal 6 karakter" required>
            ...
            <div class="col-12 col-md-4">
                ...
                <div class="form-text text-muted">Akun nonaktif ditolak masuk sistem.</div>
            </div>
        </div>

        <div class="mb-4">
            <label for="foto" class="form-label-custom">Foto Profil (Opsional)</label>
