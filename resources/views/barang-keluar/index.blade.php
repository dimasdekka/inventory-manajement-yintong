...
@section('header_title', 'Riwayat Transaksi Barang Keluar')

@section('breadcrumbs')
    <i class="fa-solid fa-angle-right" style="font-size: 10px;"></i>
    <span class="active">Barang Keluar</span>
@endsection
...
<div class="card-custom">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <!-- Pencarian & Filter -->
        <form action="{{ route('barang-keluar.index') }}" method="GET" class="d-flex flex-wrap gap-2 flex-grow-1">
            ...
            @if(request()->anyFilled(['tanggal_mulai', 'tanggal_selesai', 'barang_id']))
                <a href="{{ route('barang-keluar.index') }}" class="btn-custom btn-custom-light">
                    Reset
                </a>
            @endif
        </form>

        <!-- Catat Barang Keluar (Admin & Staff) -->
        @if(in_array(auth()->user()->role, ['administrator', 'staff_gudang']))
            <div>
                <a href="{{ route('barang-keluar.create') }}" class="btn-custom btn-custom-dark">
            ...
            <tbody>
                @forelse($barangKeluar as $item)
                    <tr>
                        ...
                        <td class="fw-semibold text-danger">-{{ $item->jumlah }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="10" class="text-center text-muted py-4">
                            Tidak ada riwayat transaksi barang keluar.
                        </td>
                    </tr>
                @endforelse
            </tbody>
